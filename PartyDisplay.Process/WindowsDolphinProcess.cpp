...
namespace PartyDisplayProcess {
	bool WindowsDolphinProcess::findPID() {
		...
		if (Process32First(snapshot, &entry) == TRUE) {
			do {
				...
				const bool match{ s_dolphinProcessName ?
					(exeFile == utf8_to_wstring(s_dolphinProcessName) || exeFile == utf8_to_wstring(s_dolphinProcessName) + L".exe") :
					(exeFile == L"Dolphin.exe" || exeFile == L"DolphinQt2.exe" || exeFile == L"DolphinWx.exe") };
#else
				const std::string exeFile{ entry.szExeFile };
				const bool match{ s_dolphinProcessName ?
					(exeFile == s_dolphinProcessName || exeFile == std::string(s_dolphinProcessName) + ".exe" :
					(exeFile == "Dolphin.exe" || exeFile == "DolphinQt2.exe" || exeFile == "DolphinWx.exe") };
#endif // UNICODE
				if (match) {
		...
		// Get the handle if Dolphin is running isnce it's required on Windows to read or write into the
		// RAM of the process and to query the RAM mapping information
		m_hDolphin = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_OPERATION | PROCESS_VM_READ | PROCESS_VM_WRITE, FALSE, m_PID);
		return true;
	}
    ...
    bool WindowsDolphinProcess::readFromRAM(const u32 offset, char* buffer, const size_t size, const bool withBSwap) {
        u64 RAMAddress = 0;
        if (m_ARAMAccessible) {
